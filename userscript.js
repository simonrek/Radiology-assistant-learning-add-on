...
ANSWER:`
  ...
  class UIManager {
    ...
    addPanelEventListeners(panel) {
      ...
      // Settings toggle
      const settingsToggle = panel.querySelector("#toggle-settings")
      if (settingsToggle) {
        settingsToggle.addEventListener("click", () => this.toggleSettings())
      }

      // Stats toggle
      const statsToggle = panel.querySelector("#toggle-stats")
      if (statsToggle) {
        statsToggle.addEventListener("click", () => this.toggleStats())
      }

      // Initialize settings content and stats
      ...
      if (!this.aiTutor.hasApiKey()) {
        ...
        content.innerHTML = `
          <div class="progress-item">
            ...
            <strong>Paragraphs:</strong> ${basicSummary.paragraphs}<br>
          </div>
          <div class="progress-item">
    ...
    displaySummary(summary, container) {
      ...
      let html = `
        <div style="margin-bottom: 10px; padding: 4px 8px; background: #0a1f3d; border-radius: 4px; font-size: 9px; color: #7f8c8d; text-align: center;">
      ...
      html += `
        <div class="progress-item">
          <strong>📄 Page Analysis</strong><br>
          <strong>Difficulty:</strong> ${summary.difficulty}<br>
          <strong>Est. Reading Time:</strong> ${summary.readingTime} min<br>
        </div>
      `
      ...
      if (summary.differentials && summary.differentials.length > 0) {
        html += `
          <div class="progress-item">
            <strong>🔍 Differential Diagnoses:</strong><br>
      ...
      if (summary.imagingApproach && summary.imagingApproach.length > 0) {
        html += `
          <div class="progress-item">
            <strong>📋 Imaging Approach:</strong><br>
      ...
      html += `
        <div class="progress-item" style="border-top: 1px solid #2c4a7c; margin-top: 15px; padding-top: 15px;">
          <strong>🔄 Generate Different Summary:</strong><br>
          <div style="margin: 10px 0; display: flex; gap: 5px; flex-wrap: wrap;">
      ...
      html += `
        <div class="progress-item" style="border-top: 1px solid #2c4a7c; margin-top: 15px; padding-top: 15px;">
          <strong>❓ Ask About This Content</strong><br>
          ...
          try {
            ...
            if (cached) {
              ...
              dataPreview[`AI_CACHE_${key.replace("ai_cache_", "")}`] = {
                ...
                age: new Date(Date.now() - parsedCache.timestamp)
                  .toISOString()
                  .substr(11, 8),
              }
            }
          ...
          aiCacheEntries.forEach(([key, data]) => {
            ...
            if (data && typeof data === "object") {
              const type = cacheKey.startsWith("summary_")
                ...
                ? "Q&A"
                : "Unknown"
              formattedData += `• ${type}: Age ${data.age}, Size ${
